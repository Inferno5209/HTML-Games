...
function loadProductDetails() {
    ...
    const categoryMap = {
        ...
        'uconn-bars': 'UConn Husky Bars',
        'everyday-favorites': 'Everyday Favorites'
    };
    document.getElementById('product-category').textContent = categoryMap[product.category];
    
    // Update product image
    const imageContainer = document.querySelector('.placeholder-image');
    if (product.image) {
        imageContainer.innerHTML = `<img src="${product.image}" alt="${product.name}" class="detail-product-image">`;
    }
    
    // Update page title
    document.title = `${product.name} - Munson's Chocolate`;
}
